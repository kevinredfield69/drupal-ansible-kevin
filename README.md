...
El direccionamiento IP de los equipos Servidores, son los siguientes a mostrarse:
...
- Deshabilitar el Network-Manager del equipo anfitrión, y una vez desactivado, escribimos la dirección IP del equipo Servidor DNS, que es el equipo Servidor "nodo1", en el fichero "/etc/resolv.conf"

- Añadir la dirección IP del equipo Servidor DNS, que es el equipo Servidor "nodo1", donde la dirección IP de dicho equipo Servidor DNS, tiene que ser escrito en la primera línea del fichero "/etc/resolv.conf", siendo el primer Servidor DNS para realizar las peticiones en el equipo anfitrión. También se puede comentar todos los equipos Servidores configurados, y solo dejar en funcionamiento el equipo Servidor DNS "nodo1".
